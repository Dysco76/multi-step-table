...
export function Table() :JSX.Element {
    ...
    function renderHeaders() {
        return headers.map(
            (header) => {
                ...
                return <th className={classes[`cell-${header.width}`]} key={header.name}>{header.name}</th>
            }
            )
    }
    ...
    return (
        <table className={classes.table}>
            ...
            <tbody>
                {people.map((person: Person) => <TableRow onRemove={handleRemove} key={person.id} person={person} hiddenFields={hiddenFields}/>)}
            </tbody>
